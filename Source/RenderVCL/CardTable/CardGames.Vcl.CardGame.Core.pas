...
unit CardGames.Vcl.CardGame.Core;
...
uses
  ...
  , Vcl.Imaging.Jpeg
  , WinApi.Messages
  , CardGames.WinApi.BackDrop
  , CardGames.WinApi.ImageArrayUnit
  , Vcl.ExtCtrls
  , CardGames.Vcl.CardGame.Engine
  ;
...
type
  ...
  private
    ...
    function DrawEngine: TDrawingEngine;
  public
  ...
  private
    ...
    procedure LoadDeck;
    procedure SetDeck(const DeckType: TDeckName);
...
begin
  ...
  if DirectoryExists('Cardsets\Standard') then
    FDeckDirectory := S + '\Cardsets'
  else
    if DirectoryExists(ExtractFilePath(ParamStr(0))+'Cardsets\Standard') then
      FDeckDirectory := ExtractFilePath(ParamStr(0))+'Cardsets'
    else
      if FileExists(S + '\Config.txt') then
      begin
        OptionList := TstringList.Create;
        try
          OptionList.LoadFromFile(S + '\Config.txt');
          FDeckDirectory := OptionList[0];
        finally
          OptionList.Free;
        end;
    ...
    //Lets look thru the environment Path directories for it.
        begin
          Result := GetEnvironmentVariable('path', Buffer, 1000);
          if Result <> 0 then
          begin
    //First add a ; to the end of the buffer. If the buffer was too small in the
    //first place this will only mess up things more.
            Count := 1000;
            repeat
              dec(Count);
            until Buffer[Count] <> '';
            ...
            repeat
              ...
              begin
                if Path[Length(Path)] <> '\' then
                  Path := Path + '\';
                if DirectoryExists(Path + 'Cardsets\Standard') then
                  FDeckDirectory := Path + 'Cardsets';
                Path := '';
              end
              else
                Path := Path + Buffer[Count];
              inc(Count);
            until (FDeckDirectory <> '') or (Count = 1000);
          end;
        end;
  if FDeckDirectory = '' then
    ...
    begin
      ...
      begin
        OptionList := TstringList.Create;
        try
          Optionlist.Add(FDeckDirectory);
          OptionList.SaveToFile(S + '\Config.txt');
        finally
          OptionList.Free;
        end;
      end
...
function TCardDeck.DrawEngine: TDrawingEngine;
begin
  Result := FOwner.DrawEngine;
end;
...
//Invert the card mask for HQDragShade.
    ...
    DrawEngine.BitBlt(FCardMaskInv.Canvas.Handle, 0, 0, FCardMask.Width, FCardMask.Height,
...
{$WARNINGS ON}
procedure TCardDeck.SetDeck(const DeckType: TDeckName);
...
begin
  if (DirectoryExists(FDeckDirectory + '\' + DeckType) or
      DirectoryExists(FAltDeckDirectory + '\' + DeckType)) and
    (DeckType <> FCardDeckName) then
  begin
    FCardDeckName := DeckType;
    FNoOfCardBacks := GetNoOfBacks;
...
//1 to 4 jockers in single, double or larger decks.
  if (Size <> FNoOfCards) and (Size <= MaxCardsInDeck) and (Size >= 52) and
    ...
    begin
      ...
      DT := FCardDeckName;
      FCardDeckName := '';
      SetDeck(DT);//Calling setdeck instead of loaddeck taking account of scale.
      CardBack := Back;
      if FaceUp then
...
begin
  ...
  FDrawEngine := TDrawingEngine.Create;
...
function TCardTable.DrawEngine: TDrawingEngine;
begin
  Result := FDrawEngine;
end;
...
begin
  if Visible and FCopyToScreen then
    ...
    DrawEngine.CopyBuffer(Canvas.Handle, X, Y, Width, Height,
...
begin
  ...
  if SaveBack then
    if ACard.FAngle = 0 then
      DrawEngine.BitBlt(ACard.FUnder.Canvas.Handle, 0,0, Deck.FCardWidth, Deck.FCardHeight,
        FBitMap.Canvas.Handle, X, Y, SRCCOPY)
     else
      begin
  ...
  //that even if rotating the back later.
        ...
        DrawEngine.BitBlt(ACard.FUnder.Canvas.Handle, 0, 0, ACard.FNW, ACard.FNH,
  ...
  begin
    if ACard.FAngle = 0 then
    begin
      DrawEngine.BitBlt(FBitMap.Canvas.Handle, X, Y, Deck.FCardWidth, Deck.FCardHeight,
...
begin
  if ACard.FAngle = 0 then
    DrawEngine.BitBlt(FBitMap.Canvas.Handle, ACard.Fx, ACard.Fy, Deck.FCardWidth,
      Deck.FCardHeight, ACard.FUnder.Canvas.Handle, 0, 0, SRCCOPY)
  else
    DrawEngine.BitBlt(FBitMap.Canvas.Handle,ACard.FNXY.X, ACard.FNXY.Y, ACard.FNW,
...
begin
  ...
  begin
    DrawEngine.BitBlt(ShadowUnder.Canvas.Handle, 0, 0, ShadowUnder.Width,
      ShadowUnder.Height, FBitMap.Canvas.Handle, X1, Y1, SRCCOPY);
    DrawEngine.BitBlt(FBitMap.Canvas.Handle, X1, Y1, Deck.CardWidth, Deck.CardHeight,
      Deck.FShadow02.Canvas.Handle, 0, 0, SRCAND);
  end else
    begin
      DrawEngine.BitBlt(ShadowUnder.Canvas.Handle, 0, 0, ShadowUnder.Width,
        ShadowUnder.Height, FBitMap.Canvas.Handle, X1, Y1, SRCCOPY);
      DrawEngine.BitBlt(Deck.FShadow02D.Canvas.Handle, 0, 0, ShadowUnder.Width,
        ShadowUnder.Height, FBitMap.Canvas.Handle, X1, Y1, SRCCOPY);
...
//We return false and a standard turnover is performed instead.
  ...
  begin
    ...
    try
      ...
      //Pick up card & shadow.
          PickUp(ACard, Deck);
          DrawEngine.BitBlt(FBitMap.Canvas.Handle, X1, Y1, ShadowUnder.Width,
          ...
          repeat
            ...
            repeat
              ...
              if j + BitMap.Height > Height then
                ...
                h := BitMap.Height;
              DrawEngine.BitBlt(FBitMap.Canvas.Handle, i, j, w, h, BitMap.Canvas.Handle, 0,
                0, SRCCOPY);
              i := i + BitMap.Width;
            until i >= Width;
            j := j + BitMap.Height;
...
begin
  ...
  DrawEngine.BitBlt(Deck.FShadow00Under.Canvas.Handle, 0, 0, Deck.FShadow00Under.Width,
    Deck.FShadow00Under.Height, FBitMap.Canvas.Handle, X1, Y1, SRCCOPY);
  DrawEngine.BitBlt(Deck.FShadow01Under.Canvas.Handle, 0, 0, Deck.FShadow01Under.Width,
    Deck.FShadow01Under.Height, FBitMap.Canvas.Handle, X2, Y2, SRCCOPY);
  if FShadowMode = smOriginal then
  begin
    DrawEngine.BitBlt(FBitMap.Canvas.Handle, X1, Y1, Deck.FShadow00.Width,
      Deck.FShadow00.Height, Deck.FShadow00.Canvas.Handle, 0, 0, SRCAND);
    DrawEngine.BitBlt(FBitMap.Canvas.Handle, X2, Y2, Deck.FShadow01.Width,
      Deck.FShadow01.Height, Deck.FShadow01.Canvas.Handle, 0, 0, SRCAND);
  end else
    begin
//New shadow.
//Get cardtable region.
      DrawEngine.BitBlt(Deck.FShadow00D.Canvas.Handle, 0, 0, Deck.FShadow00Under.Width,
        Deck.FShadow00Under.Height, FBitMap.Canvas.Handle, X1, Y1, SRCCOPY);
      DrawEngine.BitBlt(Deck.FShadow01D.Canvas.Handle, 0, 0, Deck.FShadow01Under.Width,
        Deck.FShadow01Under.Height, FBitMap.Canvas.Handle, X2, Y2, SRCCOPY);
//Shade table.
      Darken(@Deck.FShadow00D, FSoftShadow, False);
      Darken(@Deck.FShadow01D, FSoftShadow, True);
//Add mask to shadow.
      DrawEngine.BitBlt(Deck.FShadow00D.Canvas.Handle, 0, 0, Deck.FShadow00.Width,
        Deck.FShadow00.Height, Deck.FShadow00IM.Canvas.Handle, 0, 0, SrcAnd);
      DrawEngine.BitBlt(Deck.FShadow01D.Canvas.Handle, 0, 0, Deck.FShadow01.Width,
...
begin
  DrawEngine.BitBlt(FBitMap.Canvas.Handle, Deck.FShadow01Point.X, Deck.FShadow01Point.Y,
    Deck.FShadow01Under.Width, Deck.FShadow01Under.Height,
    Deck.FShadow01Under.Canvas.Handle, 0, 0, SRCCOPY);
  DrawEngine.BitBlt(FBitMap.Canvas.Handle, Deck.FShadow00Point.X, Deck.FShadow00Point.Y,
...
begin
  ...
  begin
    ...
    if Angle = 0 then
    begin
      DrawEngine.BitBlt(P^.Under.Canvas.Handle, 0, 0, Deck.FCardWidth, Deck.FCardHeight
        ,FBitMap.Canvas.Handle, X, Y, SRCCOPY);
...
//Check if its safe to use current shaded mark.
        ...
        begin
          DrawEngine.BitBlt(Deck.FHQDragShade.ShadedMark.Canvas.Handle, 0, 0,
...
//Add mask to cardtable.
        DrawEngine.BitBlt(FBitMap.Canvas.Handle, X, Y, Deck.FCardWidth,
...
//Paint shadow within mask.
  ...
  //Get inverted.
          ...
          DrawEngine.BitBlt(Deck.FHQDragShade.RotatedMarkInv.Canvas.Handle, 0, 0, P^.NW,
...
//Add mask to shadow.
  ...
  //Now store what will be under the mark.
        ...
        if P^.Under.Height < P^.NH then
          P^.Under.Height := P^.NH;
        DrawEngine.BitBlt(P^.Under.Canvas.Handle, 0, 0, P^.NW, P^.NH,
          FBitMap.Canvas.Handle, P^.NXY.X, P^.NXY.Y, SRCCOPY);
 //Add mask to cardtable.
        DrawEngine.BitBlt(FBitMap.Canvas.Handle, P^.NXY.X, P^.NXY.Y, P^.NW,
          P^.NH, Deck.FHQDragShade.RotatedMark.Canvas.Handle, 0, 0, SrcAnd);
 //Paint shadow within mask.
        DrawEngine.BitBlt(FBitMap.Canvas.Handle, P^.NXY.X, P^.NXY.Y, P^.NW, P^.NH,
          Deck.FHQDragShade.ShadedMark.Canvas.Handle, 0, 0, SrcPaint);
      end
      else
        begin
...
begin
  ...
  //As left mouse button may have been released & FCardDragging set to False.
    ...
    P := @Deck.FCardMarks[CMIndex];
    if P^.Angle = 0 then
      DrawEngine.BitBlt(FBitMap.Canvas.Handle, P^.Position.X, P^.Position.Y,
  ...
  for i := High(Deck.FCardMarks) downto Low(Deck.FCardMarks) do
    ...
    begin
      ...
      if P^.Angle = 0 then
        ...
        DrawEngine.BitBlt(FBitMap.Canvas.Handle, P^.NXY.X, P^.NXY.Y,
...
//Now store what will be under the card. If FUnder is not big enough resize it.
  ...
  DrawEngine.BitBlt(FUnder.Canvas.Handle, 0, 0, FWidth, FHeight,
...
function TSprite.DrawEngine: TDrawingEngine;
begin
  Result := FOwner.DrawEngine;
end;
...
begin
  DrawEngine.BitBlt(FOwner.FBitMap.Canvas.Handle, FNX, FNY, FWidth,
...
begin
  ...
  DrawEngine.BitBlt(FUnder.Canvas.Handle, 0, 0, FBitMap.Width, FBitMap.Height,
    b.Canvas.Handle, X, Y, SrcCopy);
  DrawEngine.BitBlt(b.Canvas.Handle, X, Y, FBitMap.Width, FBitMap.Height,
...
function TTableImage.DrawEngine: TDrawingEngine;
begin
  Result := FOwner.DrawEngine;
end;
...
begin
  b := (FOwner as TCardTable).FBitMap;
  DrawEngine.BitBlt(b.Canvas.Handle, FX, FY, FBitMap.Width, FBitMap.Height,
...
begin
  ...
  DrawEngine.BitBlt(FUnder.Canvas.Handle, 0, 0, Width, Height, b.Canvas.Handle, FX, FY,
    SrcCopy);
  case FAreaType of
    0: begin
...
function TTableArea.DrawEngine: TDrawingEngine;
begin
  Result := FOwner.DrawEngine;
end;
...
begin
  FEnabled := False;
  b := (FOwner as TCardTable).FBitMap;
  DrawEngine.BitBlt(b.Canvas.Handle, FX, FY, FUnder.Width, FUnder.Height,
    FUnder.Canvas.Handle, 0, 0, SrcCopy);
end;
