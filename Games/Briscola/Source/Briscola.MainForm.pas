...
uses
  ...
  , Vcl.Controls
  , Vcl.StdCtrls
  , CardGames.WinApi.SoundUnit, System.Actions, Vcl.ActnList, Vcl.StdActns,
  Vcl.ExtCtrls
  ;
...
type
  TMainForm = class(TForm)
    ...
    miGioco: TMenuItem;
    miOpzioni: TMenuItem;
    miChooseBack: TMenuItem;
    miChooseDeck: TMenuItem;
    miAiuto: TMenuItem;
    miNuovaPartita: TMenuItem;
    miEsci: TMenuItem;
    miInformazioni: TMenuItem;
    miRegoleDelGioco: TMenuItem;
    MessagePanel: TPanel;
    procedure CardTableCardClickEvent(ACard: TCard; Button: TMouseButton);
    procedure miChooseBackClick(Sender: TObject);
    procedure miChooseDeckClick(Sender: TObject);
...
function TMainForm.NumCardsOfPlayer( APlayer: TPlayer ): Integer;
begin
  Result := 0;
  for var I := 1 to 3 do
...
procedure TMainForm.DrawPlayerCards( APlayer: TPlayer );
var
  LCard : TCard;
begin
  for var C := 1 to 3 do
...
procedure TMainForm.DropCardSound;
begin
  {$IFDEF GLSCENE}
...
procedure TMainForm.SalvaLaCartaComeGiocata(ACard: TCard);
begin
...
procedure TMainForm.ShowTempMessage(const AMessage: string; AMilliSec: Integer = 1000);
begin
...
procedure TMainForm.WaitForHumanPlay;
begin
  gHumanMove := -1;
...
procedure TMainForm.WaitForComputerPlay( AComputerIsFirstToPlay: Boolean );
begin
...
procedure TMainForm.EseguiPartita;
var
  LPoints: Integer;
...
procedure TMainForm.FinePartita;
var
  LMessage: string;
begin
...
procedure TMainForm.DrawCardOfPlayer( ACard: TCard; APlayer: TPlayer; ACardOfPlayer: TCardOfPlayer );
begin
...
procedure TMainForm.MoveCardOfPlayer( ACard: TCard; APlayer: TPlayer; ACardOfPlayer: TCardOfPlayer );
begin
...
function TMainForm.GetC64CardNumberFromCardValue(ACardValue: TCardValue): TC64CardNumber;
begin
  case ACardValue of
    cvAce:   Result :=  1;
    cvTwo:   Result :=  2;
...
function TMainForm.GetMossaDelComputer( AComputerIsFirstToPlay: Boolean ): Integer;
type
...
procedure TMainForm.MossaDelComputer( AComputerIsFirstToPlay: Boolean );
begin
...
function TMainForm.GetPointsOfCard(ACard: TCard): Integer;
begin
...
function TMainForm.GetForceOfCard(ACard: TCard): Integer;
begin
...
procedure TMainForm.miChooseBackClick(Sender: TObject);
begin
  gCardTable.CardDeck.SelectBack;
  gCardTable.DeckOrBackChanged;
end;

procedure TMainForm.miChooseDeckClick(Sender: TObject);
begin
  gCardTable.CardDeck.SelectDeck;
  gCardTable.DeckOrBackChanged;
end;
...
function TMainForm.ChiHaVintoLaManoCorrente( AFirstCardPlayed, ASecondCardPlayed: TCard ): Integer;
var
...
function TMainForm.ClientAreaHeight: Integer;
begin
  Result := ClientHeight - MessagePanel.Height;
end;
...
procedure TMainForm.ElaboraCarteGiocate( AFirstCardPlayer: TPlayer; out ATotalPoints: Integer; out AWhoWinThePlay: TPlayer );
var
...
procedure TMainForm.EliminaCarteGiocate;
begin
...
procedure TMainForm.DistribuisciNuoveCarte( AFirstPlayerToMove: TPlayer );
var
  LNuovaCarta: TCard;
begin
...
begin
  if gPartitaIniziata then
  begin
    if MessageDlg( 'Vuoi iniziare una nuova partita?', mtCustom, [mbYes,mbNo], 0, mbNo, ['Si','No'] ) = mrYes then
      NuovaPartita;
  end
  else
    NuovaPartita;
...
begin
  ...
  begin
    ...
    gCardTable.CardDeck.DeckName := 'Bresciane'; //'C_1'; //'C_1_X2';
...
begin
  Self.Caption := Format('%s', [Application.Title]);
  Inizializzazione;
end;
...
begin
  ...
  gDeckX := Self.ClientWidth - cSpaceFromBorder - cCardWidth;
